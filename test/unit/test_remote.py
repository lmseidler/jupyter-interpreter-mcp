...
class TestGetKernelConnectionInfo:
    ...
    def test_get_kernel_connection_info_success(self):
        ...
        # Mock connection file content
        conn_file_content = json.dumps(
            {
                "shell_port": 50001,
                "iopub_port": 50002,
                "stdin_port": 50003,
    ...
    def test_get_kernel_connection_info_file_not_found(self):
        ...
        # Mock execute_code_for_output to raise JupyterExecutionError
        async def mock_execute(*args):
            raise JupyterExecutionError("File not found")

        with patch.object(client, "execute_code_for_output", side_effect=mock_execute):
            # Note: get_kernel_connection_info wraps JupyterExecutionError
            # -> JupyterConnectionError
            with pytest.raises(
